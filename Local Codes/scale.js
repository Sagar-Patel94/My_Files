...
const inputFile = "bulbul.mp4";
const outputFile = "output.mp4";

getVideoDurationInSeconds(inputFile).then((duration) => {
...
(async function () {
  try {
    ...
    for (let frameCount = 1; frameCount <= frames.length; frameCount++) {
      ...
      // console.log(frame, "11111111111111111111")
    ...
    await exec(
      `"${pathToFfmpeg}" -start_number 1 -i ${outputFolder}/%d.png -vcodec ${videoEncoder} -pix_fmt yuv420p temp/no-audio-output.mp4`
    );

    getVideoDurationInSeconds("temp/no-audio-output.mp4").then((duration) => {
      console.log("Total time of modified video:- " + duration);
    })
